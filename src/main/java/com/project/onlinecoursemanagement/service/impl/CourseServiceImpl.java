...
//        }
    ...
    public List<CourseDetailDto> getCoursesByInstructorUsername(String username) {
        // Find all instructors with the given username
        List<User> instructors = userRepository.findAllByUsername(username);

        if (instructors.isEmpty()) {
            throw new UserNotFoundException("No instructor found with username: " + username);
        }

        // Aggregate courses from all matched instructors
        List<Course> allCourses = instructors.stream()
                .flatMap(instructor -> courseRepository.findByInstructor(instructor).stream())
                .collect(Collectors.toList());
    ...
    public ResponseEntity<?> getCoursesByInstructorEmail(String email) {
        try {
            User instructor = userRepository.findByEmail(email)
